...
export default function PostDetail({ postId, onBack, onOpenAyah, onViewProfile }) {
  ...
  const handleSaveEdit = async () => {
    ...
    if (t.length < 5) { setEditError('Title must be at least 5 characters'); return; }
    if (b.length < 10) { setEditError('Body must be at least 10 characters'); return; }
    setSaving(true);
    setEditError('');
    try {
      const result = await editPost(t, b);
  ...
  if (loading && !post) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        ...
        <p className="text-xs text-white/20">Loading post...</p>
      </div>
    );
  }
  ...
  if (error || !post) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-white/50 text-sm mb-1">{error || 'Post not found'}</p>
        <p className="text-white/20 text-xs mb-4">This post may have been deleted or moved.</p>
    ...
    <div className="space-y-4">
      ...
      <div className={`rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.04] to-white/[0.01] backdrop-blur-sm shadow-xl ${typeConfig.glow}`}>
        ...
        <div className={`flex items-center gap-2 px-5 py-2.5 rounded-t-2xl border-b border-white/[0.06] ${typeConfig.bg}`}>
          ...
          {/* Pinned / Locked */}
          {post.isPinned && (
            <span className="px-2 py-0.5 rounded-full bg-yellow-500/20 border border-yellow-500/30 text-yellow-300 text-[9px] font-bold tracking-wider">PINNED</span>
          )}
          {post.isLocked && (
          ...
          <div className="ml-auto relative">
            ...
            {showActions && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-20 w-40 rounded-xl bg-slate-800 border border-white/10 shadow-xl shadow-black/30 overflow-hidden py-1">
                  {isOwner && !editing && (
                    <button onClick={handleStartEdit} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-white/60 hover:text-purple-300 hover:bg-purple-500/10 transition-colors">
                      <Icons.Edit className="w-3.5 h-3.5" /> Edit Post
                    </button>
                  )}
                  {isAuthenticated && (
                    <button onClick={() => { setShowReport(!showReport); setShowActions(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-white/60 hover:text-yellow-300 hover:bg-yellow-500/10 transition-colors">
                      <Icons.Flag className="w-3.5 h-3.5" /> Report
                    </button>
                  )}
                  {(isOwner || isAdmin) && (
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {isOwner && (
                  <span className="px-1.5 py-0.5 rounded text-[9px] bg-purple-500/15 text-purple-400 font-medium">You</span>
                )}
              </div>
            ...
            <div className="mt-3 p-4 rounded-xl bg-yellow-500/[0.04] border border-yellow-500/15 animate-[scaleIn_0.15s_ease-out]">
              {reportSuccess ? (
                <div className="flex items-center gap-2 text-green-400 text-sm">
                  <Icons.CheckCircle className="w-4 h-4" /> Report submitted. Thank you.
                </div>
              ) : (
                <>
                  <p className="text-xs text-white/40 mb-2 font-medium">Why are you reporting this post?</p>
                  ...
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => { setShowReport(false); setReportReason(''); }} className="text-xs text-white/30 hover:text-white/50 px-3 py-1.5">Cancel</button>
                    <button
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.03] to-transparent overflow-hidden">
        ...
        <div className="p-5">
          ...
          {!post.isLocked && isAuthenticated && (
            <div className="mb-5">
              <CommentInput onSubmit={(body) => handleAddComment(body)} placeholder="Share your thoughts..." />
            </div>
          )}
          ...
          {!isAuthenticated && (
            <div className="mb-5 text-center py-4 rounded-xl bg-purple-500/[0.04] border border-purple-500/15">
              ...
              <p className="text-sm text-white/40">
                <button onClick={login} className="text-purple-400 hover:text-purple-300 font-medium">Sign in</button> to join the discussion
              </p>
            </div>
          )}
          ...
          {post.isLocked && (
            <div className="mb-5 text-center py-3 rounded-xl bg-yellow-500/[0.04] border border-yellow-500/15">
              <p className="text-xs text-yellow-400/50 flex items-center justify-center gap-1.5">
                <Icons.Lock className="w-3.5 h-3.5" /> This post is locked. No new comments allowed.
              </p>
            </div>
          )}
          ...
          {comments.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-[11px] text-white/15 mt-0.5">Be the first to share your thoughts</p>
            </div>
          )}
          ...
          >
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-white/35 mb-6 leading-relaxed">
                This cannot be undone. The {deleteConfirm.type} will be permanently removed.
              </p>
