  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3" role="status" aria-label="Loading profile">
        ...
        <p className="text-xs text-white/20">Loading profile...</p>
      </div>
    );
  }
  ...
  if (error || !profile) {
    return (
      <div className="text-center py-16" role="alert">
        ...
        <p className="text-white/50 text-sm mb-1">{error || 'User not found'}</p>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.05] to-white/[0.01] overflow-hidden">
        ...
        <div className="px-5 pb-5 -mt-10">
          ...
          {/* Name + meta */}
          <div className="mt-3">
            <h1 className="text-xl text-white font-bold">{profile.name || 'Anonymous'}</h1>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: 'Posts', value: stats.postCount, icon: Icons.BookOpen, iconClass: 'text-purple-400/50' },
          { label: 'Comments', value: stats.commentCount, icon: Icons.MessageCircle, iconClass: 'text-cyan-400/50' },
          { label: 'Karma', value: stats.totalKarma, icon: Icons.TrendingUp, iconClass: 'text-emerald-400/50' },
        ].map(stat => (
          <div
            key={stat.label}
        ...
        <div className="rounded-xl border border-amber-500/10 bg-amber-500/[0.03] p-4">
          ...
          <div className="mt-3">
            ...
            <div className="text-[9px] text-white/20 mt-1 text-right">
              {leaderboard.xp % 1000}/1000 XP to next level
            </div>
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.03] to-transparent overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-3.5 border-b border-white/[0.06] bg-white/[0.02]">
          ...
          <h3 className="text-sm text-white/70 font-semibold">Recent Posts</h3>
