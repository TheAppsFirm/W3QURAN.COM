    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="p-4 space-y-4">
          ...
          {step === 1 && (
            <>
              <p className="text-sm text-white/50 mb-3">What type of post?</p>
          ...
          {step === 2 && (
            <>
              ...
              <button onClick={() => setStep(1)} className="text-xs text-purple-400 hover:text-purple-300 flex items-center gap-1">
                <Icons.ArrowLeft className="w-3 h-3" /> Change type
              </button>

              <div className="flex items-center gap-2 mb-2">
                <span className="text-lg">{POST_TYPES.find(t => t.id === postType)?.icon}</span>
                <span className="text-sm text-white/60">{POST_TYPES.find(t => t.id === postType)?.label}</span>
              </div>
              ...
              {/* Ayah References */}
              <div>
                <label className="text-xs text-white/40 block mb-1">Ayah References (optional)</label>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="e.g. 2:255 or 18:1-10"
              ...
              {availableTags.length > 0 && (
                <div>
                  <label className="text-xs text-white/40 block mb-1">Topics (max 3)</label>
              ...
              <div>
                ...
                {showQuotePicker && (
                  <div className="mt-2 p-3 rounded-xl bg-amber-500/[0.04] border border-amber-500/15 space-y-3">
                    ...
                    <div>
                      <label className="text-[10px] text-white/30 block mb-1">Translation</label>
                      <select
                    ...
                    >
                      {quoteLoading ? 'Loading...' : 'Preview Verse'}
                    </button>
                      ...
                      <div className="rounded-lg bg-amber-500/[0.06] border-l-2 border-amber-500/40 p-3 space-y-1.5">
                        ...
                        {quotePreview.translation && (
                          <p className={`text-xs leading-relaxed text-white/45 ${
                            QUOTE_TRANSLATIONS.find(t => t.id === quoteTranslation)?.langCode === 'ur' ? 'text-right' : ''
                          }`}
                            dir={QUOTE_TRANSLATIONS.find(t => t.id === quoteTranslation)?.langCode === 'ur' ? 'rtl' : 'ltr'}
                            style={QUOTE_TRANSLATIONS.find(t => t.id === quoteTranslation)?.langCode === 'ur' ? { fontFamily: "'Noto Nastaliq Urdu', serif" } : {}}
                          >
                            {quotePreview.translation}
                          </p>
                        )}
                    ...
                    <div>
                      <label className="text-[10px] text-white/30 block mb-1">Your comment or question (optional)</label>
                      <input
                        type="text"
                        value={quoteComment}
                        onChange={e => setQuoteComment(e.target.value)}
                        placeholder="e.g. What does this verse mean in context?"
                        className="w-full px-2 py-1.5 rounded-lg bg-white/5 border border-white/10 text-white text-xs
