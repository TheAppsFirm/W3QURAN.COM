...
export default function UserPopover({ user, onClose, position, onSendDMRequest, onBlock, onReport, onViewProfile }) {
  ...
  const clampedPosition = (() => {
    ...
    const left = position?.left ?? 100;
    const popW = 240;
    const popH = 280; // approx height
    const vw = typeof window !== 'undefined' ? window.innerWidth : 400;
    const vh = typeof window !== 'undefined' ? window.innerHeight : 700;
    return {
      top: Math.min(Math.max(8, top), vh - popH - 8),
      left: Math.min(Math.max(8, left), vw - popW - 8),
    };
  })();
        ...
        <div className="p-2 space-y-1">
          ...
          {onViewProfile && (
            ...
            >
              ...
              <span>{t('discussions.viewProfile', 'View Profile')}</span>
            </button>
          )}
          ...
          {!isSelf && (
            ...
            >
              ...
              <span>{t('discussions.message', 'Message')}</span>
            </button>
          )}
          ...
          {!isSelf && (
            ...
            >
              ...
              <span>{blockStep === 1 ? t('discussions.confirmBlock', 'Confirm Block?') : t('discussions.block', 'Block')}</span>
            </button>
          )}
          ...
          {!isSelf && (
            ...
            >
              ...
              <span>{t('discussions.report', 'Report')}</span>
            </button>
          )}
        </div>
      </div>
