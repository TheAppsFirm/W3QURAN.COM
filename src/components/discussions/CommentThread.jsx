...
function Comment({ comment, depth = 0, onVote, onReply, onDelete, currentUserId, isAdmin, isLocked, voting }) {
  ...
  return (
    <div className={`${depth > 0 ? 'ml-2 sm:ml-4 pl-2 sm:pl-3 border-l border-white/10' : ''}`}>
      <div className="py-2">
        {/* Comment header */}
          ...
          <>
            ...
            <div className="flex items-center gap-3 ml-7 mt-1">
              ...
              {comment.replies?.length > 0 && (
                ...
                >
                  {collapsed ? `${comment.replies.length} ${t('discussions.replies', 'replies')}` : t('discussions.collapse', 'Collapse')}
                </button>
              )}
            </div>
        ...
        {collapsed && (
          ...
          >
            [+] {comment.replies?.length || 0} {t('discussions.replies', 'replies')}
          </button>
        )}
      </div>
  ...
  if (!comments || comments.length === 0) {
    return (
      <div className="text-center py-6 text-white/30 text-sm">
        {t('discussions.noCommentsYet', 'No comments yet. Be the first to share your thoughts.')}
      </div>
    );
  }
