...
async function handleGet(request, env) {
  ...
  try {
    ...
    // Tag filter — tagSlug param must come BEFORE surahId since JOIN ? precedes WHERE ?
    if (tagSlug) {
      joinClause = ' JOIN post_tags pt_f ON pt_f.post_id = p.id JOIN discussion_tags dt_f ON dt_f.id = pt_f.tag_id AND dt_f.slug = ?';
      params.unshift(tagSlug);
    }
    ...
    const countResult = await env.DB.prepare(
      `SELECT COUNT(*) as total FROM discussion_posts p${joinClause} ${where}`
    ).bind(...params).first();
    ...
    const posts = await env.DB.prepare(`
      ...
      FROM discussion_posts p${joinClause}
      ${where}
      ORDER BY ${orderBy}
      LIMIT ? OFFSET ?
    ...
    // Fetch tags for all returned posts (graceful if tables don't exist yet)
    let postTags = {};
    if (posts.results?.length) {
      try {
        const postIds = posts.results.map(p => p.id);
        const tagsResult = await env.DB.prepare(`
          SELECT pt.post_id, dt.id, dt.name, dt.slug, dt.icon, dt.color
          FROM post_tags pt
        ...
        for (const t of (tagsResult.results || [])) {
          ...
          postTags[t.post_id].push({ id: t.id, name: t.name, slug: t.slug, icon: t.icon, color: t.color });
        }
...
async function handlePost(request, env) {
  ...
  try {
    ...
    const { surahId, postType, title, content, ayahRefs, tags } = body;
    ...
    if (filterResult.severity === 'flag') {
      ...
      // Insert tags for flagged post too
      await insertPostTags(env, postId, tags);
    ...
    // Insert tags (max 3)
    await insertPostTags(env, newPostId, tags);
...
// ─── Helper: Insert post tags ────────────────────────────────
async function insertPostTags(env, postId, tags) {
  if (!tags || !Array.isArray(tags) || tags.length === 0 || !postId) return;
  try {
    const validTagIds = tags.slice(0, 3).filter(id => Number.isInteger(id) && id > 0);
    if (validTagIds.length === 0) return;

    // Verify tag IDs exist
    const existing = await env.DB.prepare(
      `SELECT id FROM discussion_tags WHERE id IN (${validTagIds.map(() => '?').join(',')})`
    ).bind(...validTagIds).all();

    const existingIds = (existing.results || []).map(t => t.id);
    if (existingIds.length === 0) return;

    // Batch insert
    const stmts = existingIds.map(tagId =>
      env.DB.prepare('INSERT OR IGNORE INTO post_tags (post_id, tag_id) VALUES (?, ?)').bind(postId, tagId)
    );
    // Increment post_count
    const countStmts = existingIds.map(tagId =>
      env.DB.prepare('UPDATE discussion_tags SET post_count = post_count + 1 WHERE id = ?').bind(tagId)
    );

    await env.DB.batch([...stmts, ...countStmts]);
  } catch (err) {
    console.error('[Discussions] Insert tags error:', err);
  }
}
